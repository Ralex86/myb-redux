...
export default class EventList extends Component{
    ...
    render(){
        ...
        const events = this.props.events.map((event) => {
            ...
            let blocked_places = (event.confrontation_type.max_players - event.slots.confirmed > 0) ? event.slots.confirmed : event.confrontation_type.max_players
            ...
            <div className={styles.container}>
                ...
                <ul>
                    {events !== null && events.map((event) => 
                        <Event requestEventView={this.props.requestEventView} 
                            key={event.id}
                            {...event} />
                    )}
                </ul>
...
function Event(props) {
    ...
    return (
        <li className={ props.nb_places - props.blocked_places <= 0 && props.nb_places !== null ? 
        ...
        <div className={styles.badge}>
            <span >
                {props.nb_places !== null ? ( props.blocked_places + '/' + props.nb_places ) : ('∞')}
            </span>
        </div>
